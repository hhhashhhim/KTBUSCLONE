...
export default class EscPosEncoder {
  ...
  private _64printerParam: PrinterParam = {
    width: 420,
    singleCharLength: 36,
    doubleCharLength: 18,
  };
   ...
   * @param options
   */
  protected _reset(options?: any): void {
   ...
   * @returns {number}
   */
  protected get fullSingleCharLengthPerLine(): number {
   ...
   * @returns {EscPosEncoder}  Return the EscPosEncoder, for easy chaining commands
   */
  printLineFull(char: string): EscPosEncoder {
    char = char.slice(0, 1);
  ...
  printFrontDeskDishs({
    ...
    const countAndPriceLength = (bigPrice || showUnitPrice) ? 13 : 10; // 价格和个数的长度
    const countAndPriceLengthWithUnitPrice = 24; // 包含单价情况价格和个数的长度
   ...
   * @param data
   */
  table(columns, data): EscPosEncoder {
    if (this._cursor != 0) {
      this.newline();
    }
  ...
  protected fixLength(str1: string, str2: string, length: number): string {
    const spaceNum = length - this.getStrWidth(str1) - this.getStrWidth(str2);
