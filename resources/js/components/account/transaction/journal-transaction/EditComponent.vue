                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <select2
                                v-model="editData.terminal_id"
                                :options="terminalsMapped"
                                :settings="{ width: '100%', dropdownParent: '#editTransaction', placeholder: 'Select From Following' }"
                            />
                        </div>
                    </div>
                    ...
                    <div class="border p-2 mb-2">
                        ...
                        <div class="row" v-if="transactionLoop > 0">
                            ...
                            <div class="form-group col-md-2"><label>Credits <span class="text-danger">*</span></label></div>
                            <div class="form-group col-md-2"><label>Debits <span class="text-danger">*</span></label></div>
                        ...
                        <div class="row" v-for="(i, index) in transactionLoop" :key="index">
                            <div class="form-group mb-2 col-md-3">
                                <select2
                                    v-model="editData.ledgers[index]"
                                    :options="headsMapped"
                            ...
                            <div class="form-group mb-2 col-md-2">
                                <input
                                    class="form-control"
                                    type="text"
                                    :value="editData.credits[index]"
                                    :disabled="editData.debits[index] > 0"
                            ...
                            <div class="form-group mb-2 col-md-2">
                                <input
                                    class="form-control"
                                    type="text"
                                    :value="editData.debits[index]"
                                    :disabled="editData.credits[index] > 0"
                            ...
                            <div class="form-group mb-2 col-md-3">
                                <input
                                    class="form-control"
                                    type="text"
                                    :value="editData.narrations[index]"
...
<script>
export default {
    props: ['btnLoading', 'editData', 'terminals', 'heads'],
    data() {
        return {
        ...
        editData: {
            ...
            handler(newVal) {
                if (newVal && newVal.ledgers) {
                    this.finalData.credits = newVal.credits.reduce((acc, val) => acc + parseFloat(val || 0), 0);
                    this.finalData.debits = newVal.debits.reduce((acc, val) => acc + parseFloat(val || 0), 0);
    ...
    methods: {
        ...
        saveTransactionRow(event, fieldName, index) {
            if (fieldName === "first") this.editData.ledgers[index] = event.id;
            if (fieldName === "second") this.editData.credits[index] = event.target.value || 0;
            if (fieldName === "third") this.editData.debits[index] = event.target.value || 0;
            if (fieldName === "fourth") this.editData.narrations[index] = event.target.value;
            this.totalAmount();
        },
        addTransactionRow() {
            this.editData.ledgers.push("0");
            this.editData.credits.push(0);
            this.editData.debits.push(0);
            this.editData.narrations.push("");
            this.transactionLoop++;
            this.totalAmount();
        },
        removeTransactionRow(event, index) {
            this.editData.ledgers.splice(index, 1);
            this.editData.credits.splice(index, 1);
            this.editData.debits.splice(index, 1);
            this.editData.narrations.splice(index, 1);
            this.transactionLoop--;
            this.totalAmount();
        },
        totalAmount() {
            this.finalData.credits = this.editData.credits.reduce((acc, val) => acc + parseFloat(val || 0), 0);
            this.finalData.debits = this.editData.debits.reduce((acc, val) => acc + parseFloat(val || 0), 0);
            this.finalData.total_amount = this.finalData.credits - this.finalData.debits;
        },
        closeModal() {
            $(".modal").click();
