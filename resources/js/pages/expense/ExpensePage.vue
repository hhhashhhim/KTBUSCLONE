                                    ...
                                    <table class="table table-bordered table-sm">
                                        ...
                                        <tbody>
                                            <tr v-for="(item, index) in startShortages" :key="'start-' + index">
                                                ...
                                                <td>
                                                    <input v-if="isEditing(item)" type="text"
                                                ...
                                                <td>
                                                    <select v-if="isEditing(item)" v-model="item.bank_id"
                                                        class="form-control form-control-sm">
                                                ...
                                                <td class="text-nowrap">
                                                    <button v-if="!isEditing(item)" class="btn btn-sm btn-primary"
                ...
                <div class="col-12 col-md-12">
                    <div class="card p-3">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <tr v-for="(i, index) in loop" :key="index">
                                    ...
                                    <td>
                                        <input type="text" @keypress="$numberValidate($event, { dot: true })" min="0"
                                            class="form-control rounded-0" v-model.number="postData.amount[index]"
                                            @input="syncPaid(index)" />
                                    </td>
                                    <td>
                                        <input type="text" @keypress="$numberValidate($event, { dot: true })" min="0"
                                            class="form-control rounded-0" v-model.number="postData.paid[index]" />
                                    </td>
                                    <td>
                                        <input type="text" @keypress="$numberValidate($event, { dot: true })"
                                            class="form-control rounded-0" :value="balances[index]" readonly />
                                    </td>
                                    <td class="add-btn" v-if="!editAble">
                                        <button class="btn btn-outline-primary mx-2" @click="addRow">
                                            Add
                                        </button>
                                        <button class="btn btn-outline-danger" @click="removeRow($event, index)"
                                            v-if="loop != 1">
                                            Remove
                                        </button>
                                    </td>
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <button type="button" class="btn btn-outline-secondary mr-4" @click="editAble = false"
                                :disabled="loading" v-else>
                                Edit
                            </button>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                        ...
                                        <tr v-for="(item, index) in expenseHeader" :key="index">
                                            <td>
                                                <select class="form-control" v-model="expensePostData.headIds[index]"
                                                    ...
                                                    <option v-for="opt in allHeaderOptions" :key="opt.id"
                                                        :value="opt.id">
                                                        {{ opt.name }}
                                                    </option>
                                                </select>
                                            ...
                                            <td>
                                                <input type="text" @keypress="$numberValidate($event, { dot: true })"
                                                    class="form-control" v-model.number="expensePostData.values[index]"
                                                    :disabled="expenseEditAble"
                                                    @input="expenseSaveRow($event.target.value, 'second', index)" />
                                            </td>
                                        </tr>
                                    </tbody>
                                    <tfoot>
                                ...
                                <div class="d-flex justify-content-end mt-3">
                                    <button v-if="!expenseEditAble" type="button" class="btn btn-outline-success mr-4"
                                        @click="expenseAdd" :disabled="loading">
                                        {{ loading ? "Saving..." : "Save" }}
                                    </button>
                                    <button v-else type="button" class="btn btn-outline-secondary mr-4"
                                        @click="expenseEditAble = false" :disabled="loading">
                                        Edit
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer d-block pt-0">
                            <button type="button" class="btn btn-danger btn-block" data-dismiss="modal"
                                :disabled="loading" @click="updateAccount">
                                Yes
                            </button>
...
export default {
    ...
    computed: {
        ...
        profitLoss() {
            ...
            // 🔥 NEW: Start + Return ELT
            const totalStartELT = (this.startShortages || []).reduce(
                (sum, i) => sum + Number(i.elt ?? 0),
                0
            );

            const totalReturnELT = (this.returnShortages || []).reduce(
                (sum, i) => sum + Number(i.elt ?? 0),
                0
            );
