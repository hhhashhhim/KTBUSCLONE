...
class OnlineTerminalCancellationController extends Controller
{
    ...
    private function buildResponseFromLog(BookkaruApiLog $log)
    {
        ...
        return [
            ...
            'data' => [
                ...
                'source' => data_get($log->request_payload, 'source', 'Online Terminal'),
    ...
    private function buildPendingResponseFromLog(BookkaruApiLog $log)
    {
        ...
        return $this->buildPendingResponse(
            ...
            data_get($log->request_payload, 'source', 'Online Terminal'),
            $deductionPercentage,
            $refundPercentage,
    ...
    {
        $ticket = $this->bookingTicketForLog($log);
        ...
        return [
            ...
            'source' => data_get($log->request_payload, 'source', 'Online Terminal'),
            'route' => data_get($ticket, 'schedule.route.name', 'N/A'),
            'bus_date' => $busDate ?: 'N/A',
            'bus_time' => $busTime ?: 'N/A',
    ...
    private function bookingTicketForLog(BookkaruApiLog $log)
    {
        $seatNumbers = collect($log->seat_numbers ?? [])
            ->map(function ($seat) {
                return trim((string) $seat);
            })
            ->filter()
        ...
        return Ticket::withTrashed()
            ...
            ->where('invoice_id', $log->normalized_invoice_id ?: $log->invoice_id)
            ->when(!empty($seatNumbers), function ($query) use ($seatNumbers) {
                $query->whereIn('seat_no', $seatNumbers);
            })
            ->orderBy('id')
            ->first();
    }

    private function formatBookingDateTime($timestamp)
    {
        if (!$timestamp) {
            return 'N/A';
        }

        return Carbon::parse($timestamp)->format('g:i A | d-m-Y');
    }

    private function formatTimeDifference($busDate, $busTime, $requestDateTime)
    {
        if (!$busDate || !$busTime || !$requestDateTime) {
            return 'N/A';
        }

        try {
            $busDateTime = Carbon::parse($busDate . ' ' . $busTime);
            $requestDateTime = Carbon::parse($requestDateTime);
        } catch (\Throwable $e) {
            return 'N/A';
        }

        $seconds = $requestDateTime->diffInSeconds($busDateTime, false);
        $prefix = $seconds < 0 ? '-' : '';
        $seconds = abs($seconds);
